...
//*********************** 3.1 METODO update_evento() **************************************************   
    ...
    public function update_evento(){
        ...
        $sql = "UPDATE evento SET placa='$this->placa',
                                    marca=$this->marca,
                                    motor='$this->motor',
                                    chasis='$this->chasis',
    ...
    private function _get_radio($arreglo,$nombre,$defecto){
        ...
        foreach($arreglo as $etiqueta){
            ...
            <tr>
                ...
                if($defecto == NULL){
                    // OPCION PARA GRABAR UN NUEVO EVENTO (id=0)
                    $html .= '<input type="radio" value="' . $etiqueta . '" name="' . $nombre . '" checked/></td>';
                
                }else{
                    // OPCION PARA MODIFICAR UN EVENTO EXISTENTE
...
//************************************* PARTE II ****************************************************   
    ...
    public function get_form($id=NULL){
        ...
        if($id == NULL){
            ...
            $num = $res->num_rows;
            if($num==0){
                $mensaje = "tratar de actualizar el evento con id= ".$id;
                echo $this->_message_error($mensaje);
            }else{   
              ...
              // ***** TUPLA ENCONTRADA *****
                ...
                $flag = "disabled"; //enable para cambiar la foto
                $op = "update";
            }
        }
        ...
        $html = '
        <form name="evento" method="POST" action="index.php" enctype="multipart/form-data">
    ...
    public function get_list(){
        ...
        $sql = "SELECT v.id, v.placa, m.descripcion as marca, c.descripcion as color, v.anio, v.avaluo  FROM evento v, color c, marca m WHERE v.marca=m.id AND v.color=c.id;";    
        $res = $this->con->query($sql);
    ...
    public function get_detail_evento($id){
        $sql = "SELECT v.placa, m.descripcion as marca, v.motor, v.chasis, v.combustible, v.anio, c.descripcion as color, v.foto, v.avaluo  
                FROM evento v, color c, marca m 
                WHERE v.id=$id AND v.marca=m.id AND v.color=c.id;";
        $res = $this->con->query($sql);
        $row = $res->fetch_assoc(); //Saca la tupla o registros
        
        $num = $res->num_rows;

        //Si es que no existiese ningun registro debe desplegar un mensaje 
        //$mensaje = "tratar de eliminar el evento con id= ".$id;
        //echo $this->_message_error($mensaje);
        //y no debe desplegarse la tablas
        
        if($num==0){
            $mensaje = "tratar de editar el evento con id= ".$id;
            echo $this->_message_error($mensaje);
        }else{ 
                $html = '
                <table border="1" align="center">
